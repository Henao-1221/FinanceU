...
const updateGoalsList = () => {
    ...
    if (appState.goals.length === 0) {
        container.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 2rem;">
                <i class="fas fa-target" style="font-size: 3rem; color: var(--muted-foreground); margin-bottom: 1rem;"></i>
                <p>No tienes metas financieras creadas</p>
                <p style="color: var(--muted-foreground); margin-bottom: 1rem;">Define tus objetivos y haz seguimiento de tu progreso</p>
    ...
    container.innerHTML = appState.goals.map(goal => {
        ...
        return `
            <div class="goal-card">
                ...
                ${goal.description ? `<p style="color: var(--muted-foreground); margin-bottom: 1rem;">${goal.description}</p>` : ''}
